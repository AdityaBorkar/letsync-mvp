import { WsMessageSchema } from "@/(pubsub)/websockets/utils/contract.js"
...
export function connect(props: {
  ...
  const { ctx } = createWsHandlerContext({
    context,
    MsgSchema: WsMessageSchema,
    ReqManager,
    ws
  ...
  ws.onopen = async () => {
    ...
    for await (const [_, database] of context.db.entries()) {
      ...
      // const live = ws_ctx.rpc.cdc({ cursor, name })
      // live.on("record", (payload: unknown) => {
